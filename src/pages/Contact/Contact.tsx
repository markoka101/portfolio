...
import { type ModalState } from '../../components/modals/FormStatusModal/FormStatusModal';
import { submitNetlifyForm } from '../../services/api/submitNetlifyForm';
...
function Contact() {
	...
	const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			await submitNetlifyForm({ name, email, message });
			setModal({
				open: true,
				title: 'Message Sent!',
			...
			<section id="contact" className="animate-fadeIn flex flex-col justify-center">
				...
				<div className="container mx-auto my-5 flex flex-wrap px-5 py-5 sm:my-16 sm:flex-nowrap sm:py-10 md:my-16 md:py-10 lg:py-10">
					<form
						...
						className="bg-opacity-85 mt-8 flex w-full flex-col rounded-xl border-4 border-slate-500 bg-black p-5 sm:mx-auto sm:mt-0 sm:w-[500px] sm:py-8 lg:w-[650px]"
					>
						...
						<div className="relative mb-4">
							...
							<input
								...
								id="name"
								name="name"
								className="w-full rounded border border-gray-700 bg-gray-800 px-3 py-1 text-base leading-8 text-gray-100 transition-colors duration-200 ease-in-out outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900"
								onChange={(e) => {
						...
						<div className="relative mb-4">
							...
							<textarea
								id="message"
								name="message"
								className="h-32 w-full rounded border border-gray-700 bg-gray-800 px-3 py-1 text-base leading-6 text-gray-100 transition-colors duration-200 ease-in-out outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900"
								onChange={(e) => {
						...
						<button
							type="submit"
							className="translate-y-1 cursor-pointer rounded-xl border-0 bg-indigo-500 px-6 py-2 text-lg text-white transition-colors duration-300 ease-in-out hover:bg-indigo-600 focus:outline-none"
						>
							Submit
						</button>
					</form>
