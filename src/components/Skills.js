    ...
    >
      {frameworks.map((frameworks) => (
        <div key={frameworks} className="py-2 px-1 sm:w-1/2 w-3/4">
          <div className="bg-gray-800 rounded flex p-4 h-full items-center">
            <BadgeCheckIcon className="text-green-400 w-6 h-6 flex-shrink-0 mr-4" />
            <span className="font-medium text-white">{frameworks}</span>
          </div>
        </div>
      ))}
    </Transition>
      ...
      <div className="container px-5 py-10 mx-auto">
        ...
        <div className="container px-5 py-1 mx-auto">
          <div className="text-center items-center justify-center">
            <button
              className="inline-flex text-gray-50 justify-center bg-green-500 border-0 py-2 px-6 mb-2 focus:outline-none hover:bg-emerald-600 hover:text-white rounded text-lg w-3/4 transition-colors duration-300 ease-in-out"
              onClick={() => {
